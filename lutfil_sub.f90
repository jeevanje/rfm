...
! SUBROUTINES
    ...
    USE LUTINF_SUB ! Identify LUT file type, molecule and spectral range
...
! Identify LUT file type, molecule and spectral range
  CALL LUTINF ( LUNTMP, NAMLUT(1:LPT), TYP, IDXMOL, IDXISO, V1, V2, &
                FAIL, ERRMSG )
  IF ( FAIL ) RETURN
!
  ...
  DO ISPC = 1, NSPC
    ...
    IF ( V1 .LE. SPC(ISPC)%WNL+WNOTOL .AND. &
      ...
      IF ( .NOT. USEFIL ) THEN
        ...
        LFL(NLFL)%NL = 0 
        IF ( TYP .EQ. 'SVD' ) THEN
          LFL(NLFL)%LUN = 0        ! SVD-LUTs read in completely from LUNTMP
        ELSE
