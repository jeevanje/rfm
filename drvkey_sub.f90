...
!
  IF ( LUNDRV .EQ. -1 ) THEN   ! just check all expected sections found
    FAIL = .TRUE.
    IF ( CIAFLG .AND. .NOT. ANY ( KEYLST .EQ. '*CIA' ) ) THEN
      ERRMSG = 'F-DRVKEY: CIA flag requires *CIA section in driver file'
    ELSE IF ( FINFLG .AND. .NOT. ANY ( KEYLST .EQ. '*FIN' ) ) THEN
