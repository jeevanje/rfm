MODULE TABINF_SUB
CONTAINS
SUBROUTINE TABINF ( LUNLUT, NAMLUT, BINFIL, LREAD, IDXMOL, IDXISO, V1, V2, &
                    FAIL, ERRMSG )
! VERSION
...
!   Called by LUTINF.
!   If BINFIL is incorrectly specified, ie unable to interpret data, the file
!   is closed and LREAD set FALSE but not regarded as a fatal error.
!
! VARIABLE KINDS
    USE KIND_DAT 
!
...
! ARGUMENTS
    ...
    LOGICAL,       INTENT(IN)  :: BINFIL ! T=Read as binary file, F=ASCII
    LOGICAL,       INTENT(OUT) :: LREAD  ! T=file identified as TAB-LUT
...
!
  IF ( BINFIL ) THEN
    ...
    READ ( LUNLUT, ERR=800 ) RECORD
    IF ( RECORD(1:1) .NE. '!' ) GOTO 800     ! Not binary LUT file
    CALL WRTLOG ( RECORD ) 
    DO WHILE ( RECORD(1:1) .EQ. '!' ) 
      READ ( LUNLUT, IOSTAT=IOS, ERR=900 ) RECORD
    END DO
    READ ( LUNLUT, IOSTAT=IOS, ERR=900 ) RECHDR
  ELSE
    OPEN ( UNIT=LUNLUT, FILE=NAMLUT, STATUS='OLD', ACTION='READ', ERR=800 )
    ...
    END DO
    READ ( LUNLUT, '(A)', IOSTAT=IOS, ERR=900 ) RECHDR
  END IF
!
  READ ( RECORD, *, IOSTAT=IOS, ERR=900 ) RFMT   
  IF ( RFMT .NE. 1.0 ) THEN
    WRITE ( ERRMSG, '(A,F10.2)' ) &
       'F-TABINF: unrecognised LUT file format, version=', RFMT
    FAIL = .TRUE.
    RETURN
  END IF
!
...
900 CONTINUE
  ...
  IF ( FAIL ) WRITE ( ERRMSG, * ) &
    'F-TABINF: Error reading LUT file header. IOSTAT=', IOS
!
END SUBROUTINE TABINF
END MODULE TABINF_SUB
