...
!         Requires 1) CIA flag enabled, and
!                  2) No other qualifiers attached to molecule name
...
! Then check for presence of sole (cia) qualifier in QALSTR
  IF ( INDEX ( QALSTR, '(cia)' ) .GT. 0 ) THEN
    FAIL = .TRUE.
    IF ( .NOT. CIAFLG ) THEN
      ERRMSG = 'F-GASTQL: (cia) qualifier set for gas=' // & 
               MOLEC //' but CIA flag not enabled'
    ELSE IF ( QALSTR .NE. '(cia)' ) THEN
