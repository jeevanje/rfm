...
! GLOBAL DATA
    ...
    USE PHYADJ_DAT, ONLY: GRAVTY, WGTAIR  ! Gravity and Molar Weight of air
...
! LOCAL CONSTANTS
    ...
    REAL(R8), PARAMETER :: WNO = 1000.0D0 ! Wno [cm-1] for refractivity
!
! LOCAL VARIABLES
...
!
  IF ( GEOFLG ) THEN
    RFRATM = 0.0
  ELSE 
    RFRATM = REFRAC ( PREATM, TEMATM, WNO )
  END IF
!
  ...
  IF ( NPSI .GT. 0 ) THEN
    ...
    ELSE
      DO IPSI = 1, NPSI    ! NPSI = 0 unless GRA flag enabled
        RFRGRA(:,IPSI) = REFRAC ( PREGRA(:,IPSI), TEMGRA(:,IPSI), WNO ) 
      END DO
    END IF
